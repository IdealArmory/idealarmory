...
// (e.g. Savage Axis 2 with 300+ caliber/finish variants) from flooding the cap.
...
function loadSiblingUpcs(dataDir, cat) {
  const upcs = new Set();
  for (const src of ['bereli', 'impactguns', 'gunscom']) {
    try {
...
// Guarantees every item whose UPC also appears at Bereli/Impact Guns/Guns.com
...
async function main() {
  ...
  for (const cat of Object.keys(byCategory)) {
    ...
    if (bands && priorityBrands.length) {
      ...
      byCategory[cat] = bandedCapWithCrossMatch(byCategory[cat], cap, bands, priorityBrands, siblingUpcs, MODEL_VARIANT_CAPS[cat]).sort((a, b) => a.price - b.price);
